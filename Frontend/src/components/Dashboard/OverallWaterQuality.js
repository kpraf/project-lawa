...
const SAFE_CLASSES = ['Class AA', 'Class A', 'Class B', 'Class C', 'AA', 'A', 'B', 'C'];

// OverallWaterQuality component
// Accept classDescriptions as a prop (default to imported if not provided)
const OverallWaterQuality = ({
  ...
  return (
    <div className="relative flex flex-row w-full h-full font-sans">
      {/* Class Indicator: left side, only left corners rounded */}
      <div className={`overall-water-quality flex-1 h-[201px] rounded-l-2xl shadow-md flex flex-col justify-center items-center ${overallGradeColor} text-white ${className}`}>
        <div className="text-center w-full">
          <span className="block text-lg font-semibold font-sans">Station {selectedStation}:</span>          {/* Show WQI grade and value with tooltip */}          <HoverModal text={ccmeGradeDescriptions[overallGrade] || ''}>
            <span className="block text-4xl font-bold font-sans underline underline-offset-4 decoration-2 cursor-help">
              {overallGrade}
            </span>
          </HoverModal>
          <span className="block text-2xl font-bold font-sans mt-1">{wqi.toFixed(1)}</span>
          {lastUpdated && (
            <span className="block text-xs text-gray-200 mt-1 font-sans">
              As of {formatLastUpdated(lastUpdated)}
            </span>
          )}
        </div>
      </div>
      {/* Warning Square: right side, only right corners rounded */}      <div
        className={`flex flex-col justify-center items-center flex-1 h-[200px] rounded-r-2xl shadow-md relative overflow-hidden
          ${
        ...
        `}
        style={{ minWidth: 0 }}
      ><div className="flex flex-col items-center w-full h-full px-2 py-2 overflow-auto">
          {dangerousParameters.length > 0 ? (
            <>
              <div className="flex flex-row items-center mb-1">
                <FaExclamationTriangle className="text-red-500 text-2xl mr-2" />
                <span className="font-bold font-sans text-red-700 text-lg break-words">Danger!</span>
              </div>
              <span className="text-base font-sans text-red-700 mb-1 text-center break-words">Parameters exceeding guidelines:</span>
              <ul className="list-disc list-inside text-base font-sans text-red-700 w-full px-3 max-h-[70px] overflow-auto break-words">
                  ...
                  return (
                    <li key={param} className="break-words">
                      <HoverModal text={tooltipText}>
                        <Link
                          to={`/monitoring-data?station=${encodeURIComponent(selectedStation || 'unknown')}&parameter=${encodeURIComponent(param)}&fromDashboard=true`}
                          className="underline decoration-dotted cursor-pointer break-words text-red-700 hover:text-red-800 transition-colors duration-200"
                        >
                          {param}
                        </Link>
                      </HoverModal>
              ...
              {missingParameters.length > 0 && (
                <>
                  <span className="text-sm font-sans text-red-600 mt-2 text-center break-words">Missing data for {missingParameters.length} parameter{missingParameters.length > 1 ? 's' : ''}:</span>
                  <div className="text-xs font-sans text-red-500 text-center break-words max-h-[40px] overflow-auto">
            ...
            <>
              <div className="flex flex-row items-center mb-1">
                ...
                <span className="font-bold font-sans text-yellow-700 text-lg break-words">Incomplete Data</span>
              </div>
              <span className="text-base font-sans text-yellow-700 mb-1 text-center break-words">Missing data for {missingParameters.length} parameter{missingParameters.length > 1 ? 's' : ''}:</span>
              <ul className="list-disc list-inside text-sm font-sans text-yellow-700 w-full px-3 max-h-[90px] overflow-auto break-words">
                  ...
                  return (
                    <li key={param} className="break-words text-xs">
                      <HoverModal text={tooltipText}>
                        <Link
                          to={`/monitoring-data?station=${encodeURIComponent(selectedStation || 'unknown')}&parameter=${encodeURIComponent(param)}&fromDashboard=true`}
                          className="underline decoration-dotted cursor-pointer break-words text-yellow-700 hover:text-yellow-800 transition-colors duration-200"
                        >
                          {param}
                        </Link>
                      </HoverModal>
            ...
            <div className="flex flex-col items-center w-full">
              <HoverModal text={`All ${measuredParams.length} measured parameters are within acceptable limits according to environmental guidelines.\n\nMeasured Parameters: ${measuredParams.join(', ')}\n\nThis indicates good water quality conditions for the monitored aspects.`}>
                <span className="font-bold font-sans text-green-700 text-lg break-words cursor-help underline decoration-dotted">All Safe</span>
              </HoverModal>
              <span className="text-base font-sans text-green-700 text-center break-words">All measured parameters are within safe levels.</span>
              <span className="text-sm font-sans text-green-600 mt-1 text-center break-words">
                Based on {measuredParams.length} parameters
              </span>
            </div>
          ) : (
            <div className="flex flex-col items-center w-full">
              <span className="font-bold font-sans text-gray-700 text-lg break-words">No Data</span>
              <span className="text-base font-sans text-gray-700 text-center break-words">No parameter data available for analysis.</span>
            </div>
          )}
        </div>
      </div>
...
// If you want to reuse isExceedance/getGuideline in other components, move them to a shared utility file and import as needed.
