...
import { gradingSystems } from '../Dashboard/paramgrade';
import ParamCharts from '../Dashboard/ParamCharts';
...
const classColors = {
  AA: 'bg-green-200',            // Light green - matches gauge color
  A: 'bg-green-200',             // Light green - same as AA (matches gauge)
  B: 'bg-green-100',             // Lighter green - matches gauge color
  C: 'bg-green-500',             // Dark green - OPTIMAL for Laguna de Bay (matches gauge)
  D: 'bg-red-500',               // Red - Poor (matches gauge color exactly)
  Failed: 'bg-black'             // Black - Failed (matches gauge color)
};
...
const SquaresContainer = ({ selectedMarker, className, waterData: propWaterData, paramsToShow, restParams = [], compact = false, zoom = 0.89, minimized = false }) => {
  ...
  const renderParameterCategory = (categoryKey, categoryData, params) => {
    ...
    return (
      <div key={categoryKey} className={`w-full ${compact ? 'mb-1' : 'mb-4 sm:mb-6'}`}>
        {/* Category Header */}
        <div className={`w-full ${compact ? 'p-1 sm:p-2' : 'p-3 sm:p-4'} rounded-t-lg border bg-gradient-to-r ${categoryColor} ${textColor}`}>
          <HoverModal text={
            ...
            <div className="flex items-center justify-center gap-2 cursor-help">
              ...
              <div className={`flex items-center justify-center rounded-full bg-white bg-opacity-30 hover:bg-opacity-50 transition-all duration-200 ${compact ? 'w-3 h-3' : 'w-5 h-5'}`}>
                <svg className={`${compact ? 'w-2 h-2' : 'w-4 h-4'} fill-current opacity-70 hover:opacity-100 transition-opacity duration-200`} viewBox="0 0 20 20">
        ...
        {/* Parameters Grid */}
        <div className={`bg-white bg-opacity-50 rounded-b-lg border-l border-r border-b border-gray-200 ${compact ? 'p-4 sm:p-6' : 'p-6 sm:p-8'}`}>
          <div className={`grid w-full ${compact ? 'gap-6 sm:gap-8 md:gap-10' : 'gap-8 sm:gap-10 md:gap-12'} ${
            compact 
  ...
  const renderParameterCard = (param, index, categoryColor) => {
            ...
            <tbody>
              {Object.entries(parameterRanges[paramKeyForRange]).map(([classKey, [unitVal, rangeVal]], idx, arr) => {
                ...
                const getCellColor = (classKey, value) => {
                  ...
                  // If multiple classes share the same value, use the color of the best class in that group
                  if (sameValueCount > 1) {
                    // Determine the "best" (most restrictive) class that has this value
                    ...
                    // Use color based on the highest quality class that shares this value (same colors as main table)
                    if (classesWithValue.includes('AA')) {
                      return 'bg-green-200'; // Same as A - Light green
                    } else if (classesWithValue.includes('A')) {
                      return 'bg-green-200'; // Same as AA - Light green
                    } else if (classesWithValue.includes('B')) {
                      return 'bg-green-100'; // Lightest green
                    } else if (classesWithValue.includes('C')) {
                      return 'bg-green-500'; // Same as C - Darkest green
                    } else if (classesWithValue.includes('D')) {
                      return 'bg-red-500'; // Same as D - Red
                    }
                  }
                  
                  // Use distinct colors for unique values (exact same as GradeDescription.js)
                  switch (classKey) {
                    case 'AA': return 'bg-green-200 font-medium'; // Same as A - Light green 
                    case 'A': return 'bg-green-200'; // Light green - High Quality
                    case 'B': return 'bg-green-100'; // Lightest green 
                    case 'C': return 'bg-green-500 font-bold text-white'; // Darkest green - Optimal (Laguna de Bay target)
                    case 'D': return 'bg-red-500'; // Red - Poor
                    default: return '';
                  }
                };
              ...
              <tr className={grade === 'Failed' ? 'ring-2 ring-blue-500 ring-inset font-bold' : ''}>
                ...
                <td className={`px-3 py-2 text-center rounded-br-lg bg-black text-white ${grade === 'Failed' ? 'ring-2 ring-blue-500 ring-inset font-bold' : ''}`}>
                  Out of range
                </td>
              </tr>
            </tbody>
    ...
    return (
      <div
        key={`${param}-${index}`}
        className={`
          bg-white bg-opacity-90 backdrop-blur-sm shadow-md rounded-lg w-full flex flex-col items-center justify-between
          transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-lg hover:bg-opacity-100
          ${isDangerous ? 'border-2 border-red-500 bg-red-50 shadow-red-200' : `border ${borderColor} ${hoverBorderColor}`}
          cursor-pointer ${paramData && value !== undefined && value !== null ? 'opacity-100' : 'opacity-75'}
          ${compact 
            ? 'p-2 xs:p-3 min-h-[110px] xs:min-h-[120px] sm:min-h-[130px] md:min-h-[140px] min-w-[100px] xs:min-w-[120px] sm:min-w-[140px]' 
            : 'p-3 sm:p-4 md:p-5 lg:p-6 min-h-[160px] sm:min-h-[180px] md:min-h-[200px] lg:min-h-[220px] xl:min-h-[240px] min-w-[160px] sm:min-w-[180px] md:min-w-[200px]'
          }
          font-sans group relative
        `}
        style={{ display: 'flex', flexDirection: 'column', height: '100%', alignItems: 'center', justifyContent: 'flex-start', paddingLeft: '5px' }}
      >
        <div className="w-full flex items-center justify-center flex-shrink-0 relative z-10 mb-1">
          <ParamCharts parameterData={{ ...(paramData || {}), param }} parameterRanges={parameterRanges} compact={compact} hideClassIndicator={true} />
        </div>
        
        {/* Remove this grade display if it exists */}
        {/* {grade && grade !== 'Unknown' && (
        ...
        <div className="flex flex-col items-center justify-center w-full absolute bottom-0 left-0 right-0 px-1 pb-1 z-20">
          ...
          <div className={`${compact ? 'text-sm xs:text-base sm:text-lg font-bold' : 'text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold'} text-gray-800 text-center leading-tight mb-1`}>
            {value !== undefined && value !== null ? (typeof value === 'number' ? value.toFixed(1) : value) : "No data"}
          </div>
          
          {/* Parameter name with tooltip */}
          <HoverModal text={hoverContent}>
            <p className={`${compact ? 'text-xs sm:text-xs leading-tight' : 'text-xs sm:text-sm md:text-base lg:text-lg'} font-bold font-sans text-gray-800 cursor-help text-center leading-tight group-hover:text-gray-900 mt-1 relative hover:text-blue-600 transition-colors duration-200 px-1`}>
              {compact ? (
                param.length > 20 ? param.replace(/\s+/g, ' ').substring(0, 20) + '...' : param
              ) : param}
              <span className={`absolute -top-1 -right-1 ${compact ? 'w-0.5 h-0.5' : 'w-1 h-1'} rounded-full bg-blue-400 opacity-0 group-hover:opacity-100 transition-opacity duration-200`}></span>
            </p>
          </HoverModal>
          
          {/* Units - only show if not compact or essential */}
          {(!compact || (compact && unit && unit.length < 8)) && (
          ...
          {(!compact || (compact && paramDate)) && (
            <p
              className={`font-sans text-center mt-1 leading-tight transition-all duration-300 ${compact ? 'truncate max-w-full pb-6 text-xs' : 'pb-6 sm:pb-8 md:pb-10 lg:pb-12 xl:pb-14 text-base'} group-hover:scale-110 group-hover:shadow-lg text-black`}
  ...
  return (
    <div className={`${className} flex flex-col w-full h-full ${minimized || compact ? 'overflow-hidden' : 'overflow-auto'}`} style={{ zoom }}>
      <div className={`flex-1 min-h-0 ${minimized || compact ? 'overflow-hidden' : 'overflow-auto'}`}>
